...
class WarpTests: XCTestCase {
    ...
    func testModuleMult() throws {
        ...
        try Device.current.sync {
            ...
            /// Launch kernel
            try mult<<<(8, 32)>>>[.float(5.0), .pointer(to: &x), .int(256)]
        }
    ...
    func testModuleSaxpy() throws {
        try Device.current.sync {
            ...
            let x = DeviceArray<Float>(Array(sequence(first: 0, next: {$0+1}).prefix(512)))
            let y = DeviceArray<Float>(Array(sequence(first: 512, next: {$0-1}).prefix(512)))
            var out = DeviceArray<Float>(capacity: 512)

            /// Launch kernel
            try saxpy<<<(32, 128)>>>[
                .float(5.1), .constPointer(to: x), .constPointer(to: y), .pointer(to: &out), .int(512)
            ]
    ...
    func testModuleValueReference() throws {
        try Device.current.sync {
            let source =
                "extern \"C\" __global__ void sum(const float *x, int count, float *out) {"
            ...
            /// Launch kernel
            try saxpy<<<(1, 1)>>>[.constPointer(to: x), .int(Int32(x.count)), .pointer(to: &out)]
    ...
    static var allTests : [(String, (WarpTests) -> () throws -> Void)] {
        return [
            ...
            ("testModuleValueReference", testModuleValueReference),
            ("testCollectionOperations", testCollectionOperations),
        ]
    }
